...
async function changeThreadName(api, title, event) {
  isChanging = true;
  stopChange = false;

  while (!stopChange) {
    await sleep(1500); // Adjust the time interval here
    if (!stopChange) {
      const randomIndex = Math.floor(Math.random() * randomNames.length);
      const randomTitle = replaceNamePlaceholder(
        randomNames[randomIndex],
        title,
      );
      await setTitle(api, randomTitle, event.threadID);
    }
  }
...
export const command: Command = {
  ...
  execute: async (api, event, args) => {
    if (event.isGroup) {
      if (args[0] === "stop") {
        stopChange = true;
        return api.sendMessage("Dừng reo tên thành công.", event.threadID);
      }

      if (isChanging) {
        return api.sendMessage("Đang reo đây thk óc:))", event.threadID);
      }

      const title = args.join(" ");
      if (!title) {
        return api.sendMessage("Nhập tên vào thk culi!", event.threadID);
      }

      await changeThreadName(api, title, event);

      return api.sendMessage(
        `${title} ngu đã rơi vào tầm ngắm chuẩn bị đi thằng Culi😏`,
        event.threadID,
      );
    } else {
      return api.sendMessage(
        "Chỉ có thể dùng ở box thôi thk đần.",
        event.threadID,
      );
    }
  },
